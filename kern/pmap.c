...
mem_init(void)
{
    ...
    boot_map_region(kern_pgdir, UPAGES,
                    (int)ROUNDUP(npages*sizeof(struct PageInfo), PGSIZE),
                    PADDR(pages),
                    PTE_U);
    boot_map_region(kern_pgdir, pages,
                    (int)ROUNDUP(npages*sizeof(struct PageInfo), PGSIZE),
                    PADDR(pages),
                    PTE_W);
    ...
    boot_map_region(kern_pgdir, UENVS,
                    (int)ROUNDUP(NENV * sizeof(struct Env), PGSIZE),
                    PADDR(envs),
                    PTE_U);
    boot_map_region(kern_pgdir, envs,
                    (int)ROUNDUP(NENV * sizeof(struct Env), PGSIZE),
                    PADDR(envs),
                    PTE_W);
...
page_init(void)
{
    ...
    for(;i < (size_t)ROUNDUP(EXTPHYSMEM+boot_alloc(0)-KERNBASE, PGSIZE)/PGSIZE;i++){
        pages[i].pp_ref = 1;
...
boot_map_region(pde_t *pgdir, uintptr_t va, size_t size, physaddr_t pa, int perm)
{
    for(size_t i = 0; i < size; i += PGSIZE){
        ...
        *pte = (PGNUM(pa + i) << PGSHIFT) | perm | PTE_P;
    }
}
